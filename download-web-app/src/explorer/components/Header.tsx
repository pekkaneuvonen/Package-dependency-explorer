...
interface IProps {
	...
	crumbClickHandler: (index: number) => void
}

const Header: React.FunctionComponent<IProps> = observer((props: IProps) => {

	const appState = useContext(AppState)
	const { breadcrumbs, currentPackage } = appState;

	const latestRef = useRef<HTMLDivElement>(null);
	const crumbContent = useRef<HTMLDivElement>(null);
	const crumbContainer = useRef<HTMLDivElement>(null);


	const crumbButtonFactory = (crumbIndex: number) => {
		if (crumbIndex !== breadcrumbs.length-1) {
			return (event: any) => {
				return props.crumbClickHandler(crumbIndex);
			}
		} else {
	...
	useLayoutEffect(() => {
		if (breadcrumbs.length > 0) {
			if (crumbContent.current && crumbContainer.current) {
				crumbContainer.current.scrollTo(0, crumbContent.current.clientHeight - crumbContainer.current.clientHeight); 
			}
		}
	}, [ breadcrumbs ])
		...
		<div className="Header">
			<div className="Header-column">
				<div className="appId">
					...
					<p className="appId-description">Online UI demo</p>
			...
			<div className="Header-column">
					<div className={currentPackage && breadcrumbs.length > 0 ? "Header-column-top Header-column-top-middle appear_up" : "Header-column-top Header-column-top-middle hidden_down"}>
						<button className="Header-column-infoButton element_button" onClick={props.homeClickHandler}>Reset to root</button>
						<div className="Header-breadcrumbs-container" ref={crumbContainer}>
							<div className="breadcrumbs_content" ref={crumbContent}>
								{breadcrumbs.map((crumb: Pointer, indx: number) => {
									return <button key={indx} className={ indx === breadcrumbs.length-1 ? "breadCrumb element_button current-crumb" : "breadCrumb element_button"} onClick={crumbButtonFactory(indx)}>
										<div className="breadcrumbDivCircle upper_div_circle"/>
										<p ref={indx === breadcrumbs.length-1 ? latestRef : null} >{crumb.id}</p>
									</button>
								})}
							</div>
						</div>
